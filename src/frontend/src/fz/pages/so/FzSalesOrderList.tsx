...
/**
 * Branch-scoped sales order list with search, status chips and
 * an outstanding-only toggle. Row click opens the single-page detail.
 */
export default function FzSalesOrderList() {
  ...
  const { data, isLoading } = useBranchQuery({
    key: ['sales-orders', debouncedSearch, outstanding, page],
    ...
    <Stack>
      ...
      <Group justify='space-between'>
        ...
        <Button
          ...
          disabled={!user.hasAddRole(UserRoles.sales_order)}
          data-testid='fz-so-create'
        >
          New order
        </Button>
      </Group>
      <Group>
        <TextInput
          placeholder='Search by reference, customer, description...'
      ...
      {isLoading ? (
        ...
        <Center h='30vh'>
          <Stack align='center' gap='xs'>
            <Text fw={600}>No sales orders</Text>
        ...
        <>
          <Table highlightOnHover data-testid='fz-so-table'>
            <Table.Thead>
              <Table.Tr>
                ...
                <Table.Th>Total (incl. tax)</Table.Th>
            ...
            <Table.Tbody>
              {orders.map((order) => (
                <Table.Tr
                  ...
                  onClick={() => navigate(`/b/so/${order.pk}`)}
                >
                  <Table.Td>
                    <Text size='sm' fw={600}>
                      {order.reference}
                  ...
                  <Table.Td>
                    <Text size='sm'>{order.customer_detail?.name ?? '-'}</Text>
                  </Table.Td>
                  <Table.Td>
                    <StatusRenderer
