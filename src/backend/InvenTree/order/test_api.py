            ...
            for _idx in range(10):
                new_orders.append(
                    models.PurchaseOrder(
                        supplier=supplier, reference=f'PO-{idx + 100}', tenant=tenant
                    )
                )
...
class TransferOrderTest(OrderTest):
    ...
    def test_transfer_order_tenant_validation(self):
        """Test that transfer orders enforce tenant rules on locations.

        - Source and destination locations must belong to the same tenant
        - Locations must belong to the order's tenant
        """
        ...
        # API-created orders are assigned to the default tenant
        tenant1 = Tenant.objects.first()
        tenant2 = Tenant.objects.create(name='Other Tenant', code='OTHER')

        loc_t1_a = StockLocation.objects.create(name='T1 Source', tenant=tenant1)
        loc_t1_b = StockLocation.objects.create(name='T1 Dest', tenant=tenant1)
        loc_t2_a = StockLocation.objects.create(name='T2 Source', tenant=tenant2)
        loc_t2_b = StockLocation.objects.create(name='T2 Dest', tenant=tenant2)

        url = reverse('api-transfer-order-list')

        # Locations in different tenants are rejected
        response = self.post(
            url,
            {
                'reference': 'TO-99001',
                'description': 'Cross tenant transfer',
                'take_from': loc_t1_a.pk,
                'destination': loc_t2_b.pk,
            },
            expected_code=400,
        )

        self.assertIn('does not match order tenant', str(response.data))

        # Even a tenant-less order (not possible via the API) must not
        # transfer between locations of different tenants
        order = models.TransferOrder(
            reference='TO-99999',
            description='Tenant-less cross tenant transfer',
            take_from=loc_t1_a,
            destination=loc_t2_b,
        )

        with self.assertRaises(ValidationError) as cm:
            order.full_clean()

        self.assertIn('must belong to the same tenant', str(cm.exception))

        # Locations which do not match the order tenant are rejected
        response = self.post(
            url,
            {
                'reference': 'TO-99002',
                'description': 'Wrong tenant transfer',
                'take_from': loc_t2_a.pk,
                'destination': loc_t2_b.pk,
            },
            expected_code=400,
        )
        ...
        # Same-tenant transfer is allowed
        response = self.post(
            url,
            {
                'reference': 'TO-99003',
                ...
                'take_from': loc_t1_a.pk,
                'destination': loc_t1_b.pk,
            },
            expected_code=201,
        )

        # Changing a location to another tenant's location is rejected
        detail_url = reverse(
            'api-transfer-order-detail', kwargs={'pk': response.data['pk']}
        )

        response = self.patch(
            detail_url, {'destination': loc_t2_b.pk}, expected_code=400
        )
...
class TransferOrderAllocateTest(OrderTest):
    ...
    def test_allocate_cross_tenant(self):
        """Test that stock in another tenant's location cannot be allocated."""
        from stock.models import StockLocation
        from tenant.models import Tenant
        ...
        other_location = StockLocation.objects.create(
            name='Other tenant location', tenant=tenant2
        )

        # Assign the order to the default tenant
        ...
        item = StockItem.objects.create(
            part=line.part, quantity=50, location=other_location
        )

        response = self.post(
            self.url,
            {'items': [{'line_item': line.pk, 'stock_item': item.pk, 'quantity': 5}]},
            expected_code=400,
        )

        self.assertIn(
            'Stock location tenant does not match order tenant', str(response.data)
        )
